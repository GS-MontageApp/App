// ============================================================================
// ZANGENSCHLOSSER-APP: SHELL / GLOBAL UI CONTROLLER (v1.12.3)
// ============================================================================
window.AppShell = (() => {
  function checkDailySplash() {
    // Nutzt jetzt konsequent den v1 Key
    const today = new Date().toISOString().split('T')[0];
    const lastSplash = localStorage.getItem('zangenschlosser_splash_date_v1');
  ...
  function openSubModal(type) {
    ...
    if (type.startsWith('logbook-')) {
      ...
      const logs = (window.allLogbooks && window.allLogbooks[key]) ? window.allLogbooks[key] : [
        { version: "v1.12.3", date: "20.09.2026, 19:00 (MEZ)", text: "Aktualisierung auf Version v1.12.3 durchgeführt. Splash-Screen-Bug behoben.", border: "border-indigo-500" }
      ];
      ...
      contentEl.innerHTML = `
        <div class="space-y-3 text-slate-700 text-sm">
          ...
          <p class="text-xs text-slate-500">Version: v1.12.3</p>
        </div>
      `;
    }
